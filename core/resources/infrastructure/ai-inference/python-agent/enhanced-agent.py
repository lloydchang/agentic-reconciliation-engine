#!/usr/bin/env python3
"""
Enhanced Always-On Memory Agent with Internet Search
Qwen2.5 0.5B via Ollama + DuckDuckGo Search for AI Grounding

This agent can search the internet for information it doesn't know,
providing real-time grounding for queries about current AI trends,
technical details, and emerging technologies.
"""
...
Be thorough but concise."""
    ...
    def query(self, question: str, use_search: bool = True) -> str:
        """Answer questions using stored memories and optional internet search."""
        memories = read_all_memories()
        consolidations = read_consolidation_history()

        context = "Stored Memories:\n"
        for m in memories["memories"][:20]:
        ...
        # Decide if we need to search the internet
        search_needed = False
        if use_search:
            search_prompt = f"""Based on this question and available memories, do we need current web information?

Question: {question}

Available memories cover: {[m['summary'][:50] for m in memories["memories"][:10]]}

Should we search the internet for additional grounding? Answer only YES or NO."""

            search_decision = ollama_generate(search_prompt).strip().upper()
            search_needed = "YES" in search_decision

        search_results = ""
        if search_needed:
            log.info(f"🔍 Searching web for: {question}")
            search_results = search_web(question)
            if search_results:
                search_results = "\n\nWeb Search Results:\n" + "\n".join([
                    f"- {r['title']}: {r['body'][:200]}... ({r['url']})"
                    for r in search_results
                ])
                log.info(f"📊 Found {len(search_results)} search results")

        prompt = f"""You are a Memory Query Agent. Answer this question using stored memories and web search results if available.

Question: {question}

{context}{search_results}

Provide a comprehensive answer. Reference memory IDs like [1], [2]. If web search was used, cite sources. Be thorough but concise."""
...
- Internet Search: Available via DuckDuckGo"""
...
def build_http(agent: MemoryAgent, watch_path: str = "./inbox"):
    ...
    async def handle_query(request: web.Request):
        ...
        use_search = request.query.get("search", "true").lower() == "true"
        if not q:
            return web.json_response({"error": "missing ?q= parameter"}, status=400)
        answer = agent.query(q, use_search=use_search)
        return web.json_response({"question": q, "answer": answer, "search_used": use_search})
...
# Consolidation timer (same as before)
...
async def main_async(args):
    ...
    log.info("🧠 Enhanced Always-On Memory Agent starting")
    ...
    log.info(f"   Internet Search: Enabled via DuckDuckGo")
    ...
    log.info(f"   Internet search available for enhanced responses")
    log.info("")
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced Always-On Memory Agent - Qwen2.5 0.5B + Internet Search")
