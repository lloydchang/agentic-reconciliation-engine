# {{overlay-name}} Skill Overlay

## Overview

This is a skill overlay template that extends the base `{{base-skill}}` skill with additional capabilities while maintaining agentskills.io compliance.
...
cp -r core/deployment/overlays/templates/skill-overlay core/deployment/overlays/core/ai/skills/{{overlay-name}}

# Customize overlay configuration
cd core/deployment/overlays/core/ai/skills/{{overlay-name}}
...
- **Base Skill**: {{base-skill}} {{base-version}}+
- **agentskills.io**: 1.0+
- **Python**: 3.8+
- **Kubernetes**: 1.20+
