...
DELETED_FILES=$(git diff --name-only --diff-filter=D origin/main...HEAD -- 'core/resources/tenants/**/*.yaml')
...
CHANGED_FILES=$(git diff --name-only --diff-filter=M origin/main...HEAD -- 'core/resources/tenants/**/*.yaml')
