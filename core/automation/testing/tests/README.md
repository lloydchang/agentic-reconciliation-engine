...
./core/automation/testing/test-local-suite.sh

# Force cluster type if needed
CLUSTER_TYPE=minikube ./core/automation/testing/test-local-suite.sh
```

#### `test-components.sh` - Individual Component Testing
Quick validation of specific components without full suite:
```bash
./core/automation/testing/test-components.sh flux         # Flux GitOps controllers
./core/automation/testing/test-components.sh emulators    # Cloud service emulators
./core/automation/testing/test-components.sh workloads    # Application deployments
./core/automation/testing/test-components.sh security     # Security policies and controls
./core/automation/testing/test-components.sh networking   # Ingress and service connectivity
./core/automation/testing/test-components.sh all          # Run all component tests
```
...
- **Deployment**: `kubectl apply -k core/resources/tenants/aws/eks-emulator/`

#### AKS Emulation (Azure)
- **Tool**: Azure AKS emulator in LocalStack
- **Features**: AKS API simulation
- **Deployment**: `kubectl apply -k core/resources/tenants/azure/aks-emulator/`

#### GKE Emulation (GCP)
- **Tool**: GKE emulator with Minikube + GCP auth
- **Features**: GKE API simulation
- **Deployment**: `kubectl apply -k core/resources/tenants/gcp/gke-emulator/`
...
./core/automation/testing/test-components.sh workloads
...
1. Create test script in `core/automation/testing/` directory
