...
# Check for key files
files=("README.md" "SETUP.md" "core/core/automation/ci-cd/scripts/deploy-agentic-reconciliation-engine.sh")
...
for flux_file in "${flux_files[@]}"; do
    if [ -f "/app/core/operators/flux/$flux_file" ]; then
        pass_step "Flux file exists: $flux_file"
        
        # Basic Flux validation
        if grep -q "apiVersion:" "/app/core/operators/flux/$flux_file"; then
