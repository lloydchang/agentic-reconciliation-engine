...
mkdir -p core/automation/testing/{unit,integration,e2e,performance,security}

# Create unit tests
echo -e "${YELLOW}🔬 Creating unit tests...${NC}"

cat > core/automation/testing/unit/flux-components.test.sh << 'EOF'
#!/bin/bash
...
cat > core/automation/testing/integration/gitops-integration.test.sh << 'EOF'
#!/bin/bash
...
cat > core/automation/testing/e2e/full-gitops-pipeline.test.sh << 'EOF'
#!/bin/bash
...
cat > core/automation/testing/performance/performance-benchmarks.test.sh << 'EOF'
#!/bin/bash
...
cat > core/automation/testing/security/security-compliance.test.sh << 'EOF'
#!/bin/bash
...
# Unit tests
((total_suites++))
if ! run_test_suite "Unit" "core/automation/testing/unit/flux-components.test.sh"; then
    ((failed_suites++))
fi

# Integration tests
((total_suites++))
if ! run_test_suite "Integration" "core/automation/testing/integration/gitops-integration.test.sh"; then
    ((failed_suites++))
fi

# End-to-end tests
((total_suites++))
if ! run_test_suite "E2E" "core/automation/testing/e2e/full-gitops-pipeline.test.sh"; then
    ((failed_suites++))
fi

# Performance tests
((total_suites++))
if ! run_test_suite "Performance" "core/automation/testing/performance/performance-benchmarks.test.sh"; then
    ((failed_suites++))
fi

# Security tests
((total_suites++))
if ! run_test_suite "Security" "core/automation/testing/security/security-compliance.test.sh"; then
    ((failed_suites++))
fi
...
chmod +x core/automation/testing/unit/*.sh
chmod +x core/automation/testing/integration/*.sh
