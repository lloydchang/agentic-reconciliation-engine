#!/bin/bash
# Agentic Reconciliation Engine - Quick Start Script (Local Kind)
# Environment-specific quickstart for local Kind clusters
...
# Help function
show_help() {
    echo "Agentic Reconciliation Engine - Quick Start (Local Kind)"
    echo ""
    ...
    echo "  using local Kind Kubernetes cluster."
    echo ""
    echo "  This includes:"
    echo "  - Kind cluster creation and configuration"
    ...
    echo ""
    echo "EXAMPLES:"
    echo "  $0                                    # Standard Kind setup with AI agents dashboard"
    ...
    echo ""
    echo "OVERLAY USAGE:"
    echo "  For overlay-specific operations, use: overlay-quickstart-local-kind.sh"
