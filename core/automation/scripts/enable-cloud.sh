...
subdirectory `core/operators/flux/cloud-<provider>` to the kustomization
...
KUSTOMIZATION_FILE="$REPO_ROOT/core/operators/flux/kustomization.yaml"
OVERLAY_DIR="$REPO_ROOT/core/operators/flux/cloud-$PROVIDER"
OVERLAY_ENTRY="  - ./cloud-$PROVIDER"

if [[ ! -d "$OVERLAY_DIR" ]]; then
  echo "Overlay directory not found: $OVERLAY_DIR" >&2
  echo "Create it first (e.g., adapt core/operators/flux/cloud-aws)" >&2
  exit 1
fi
...
if [[ "$PROVIDER" == "azure" && -n "$EMULATOR_ACTION" ]]; then
  TOGGLE_COMMANDS+=("core/operators/flux/cloud-azure/kustomization.yaml|  - local-emulator|$EMULATOR_ACTION|patchesStrategicMerge")
fi
