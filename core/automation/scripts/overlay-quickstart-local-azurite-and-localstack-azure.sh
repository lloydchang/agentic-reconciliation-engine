#!/bin/bash
# Agentic Reconciliation Engine - Overlay Quick Start Script (Local Azurite and LocalStack Azure)
# Environment-specific overlay quickstart for local Azure emulators
...
# Help function
show_help() {
    echo "Agentic Reconciliation Engine - Overlay Quick Start (Local Azurite and LocalStack Azure)"
    echo ""
    ...
    echo ""
    echo "DESCRIPTION:"
    echo "  Overlay quick start for Agentic Reconciliation Engine with Azure emulators."
    echo "  Demonstrates overlay pattern where base script (quickstart-local-azurite-and-localstack-azure.sh) is"
    echo "  extended without modification through environment variables and hooks."
    echo ""
    echo "  This includes:"
    echo "  - Azurite Azure Storage emulator setup"
    echo "  - LocalStack Azure services emulator setup"
    ...
    echo ""
    echo "EXAMPLES:"
    echo "  $0 --all              # Complete Azure emulator overlay quick start"
    ...
    echo ""
    echo "AZURE EMULATOR CONFIGURATION:"
    echo "  - Azurite: Azure Storage emulator (ports 10000-10002)"
    echo "  - LocalStack: Azure services emulator (ports 4566-4571)"
    echo "  - Automatic container startup and configuration"
    echo "  - Azure environment variables automatically set"
    echo ""
    ...
    echo "  3. Runs base quickstart-local-azurite-and-localstack-azure.sh (never modifying it)"
