...
create_age_example() {
    ...
    cat > overlay/examples/sops/example-secret-age.yaml << 'EOF'
apiVersion: v1
kind: Secret
metadata:
...
EOF
    ...
    if [ -f "age.agekey" ]; then
        ...
        sops --encrypt --in-place overlay/examples/sops/example-secret-age.yaml
        print_success "Age encryption example created"
    else
        print_warning "age.agekey not found - skipping encryption"
        mv overlay/examples/sops/example-secret-age.yaml overlay/examples/sops/example-secret-age.unencrypted.yaml
    fi
}
...
create_gpg_example() {
    ...
    # Create sample secret
    cat > overlay/examples/sops/example-secret-gpg.yaml << 'EOF'
apiVersion: v1
kind: Secret
metadata:
...
EOF
    ...
    mv overlay/examples/sops/example-secret-gpg.yaml overlay/examples/sops/example-secret-gpg.unencrypted.yaml
...
create_kms_examples() {
    ...
    # AWS KMS example
    cat > overlay/examples/sops/example-secret-aws-kms.yaml << 'EOF'
apiVersion: v1
kind: Secret
metadata:
...
EOF
    ...
    # GCP KMS example
    cat > overlay/examples/sops/example-secret-gcp-kms.yaml << 'EOF'
apiVersion: v1
kind: Secret
metadata:
...
EOF
    ...
    cat > overlay/examples/sops/example-secret-azure-kv.yaml << 'EOF'
apiVersion: v1
kind: Secret
metadata:
...
create_sops_configs() {
    ...
    # Age configuration
    cat > overlay/examples/sops/.sops.yaml.age << 'EOF'
creation_rules:
  - path_regex: .*\.yaml$
...
EOF
    ...
    cat > overlay/examples/sops/.sops.yaml.gpg << 'EOF'
creation_rules:
  - path_regex: .*\.yaml$
...
EOF
    ...
    cat > overlay/examples/sops/.sops.yaml.aws-kms << 'EOF'
creation_rules:
  - path_regex: .*\.yaml$
...
EOF
    ...
    cat > overlay/examples/sops/.sops.yaml.gcp-kms << 'EOF'
creation_rules:
  - path_regex: .*\.yaml$
...
EOF
    ...
    cat > overlay/examples/sops/.sops.yaml.azure-kv << 'EOF'
creation_rules:
  - path_regex: .*\.yaml$
...
create_documentation() {
    ...
    cat > overlay/examples/sops/README.md << 'EOF'
...
main() {
    ...
    print_status "Examples are available in: overlay/examples/sops/"
}
