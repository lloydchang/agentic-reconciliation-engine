...
build_images() {
    ...
    docker build -f core/deployment/ai-agent-evaluator-Dockerfile -t "${DOCKER_REGISTRY}/ai-agent-evaluator:${IMAGE_TAG}" --target base .
    
    # Build API server image
    print_info "Building API server image..."
    docker build -f core/deployment/ai-agent-evaluator-Dockerfile -t "${DOCKER_REGISTRY}/ai-agent-evaluator-api:${IMAGE_TAG}" --target production .
    
    # Build Temporal worker image
    print_info "Building Temporal worker image..."
    docker build -f core/deployment/ai-agent-evaluator-Dockerfile -t "${DOCKER_REGISTRY}/ai-agent-evaluator:${IMAGE_TAG}" --target temporal-worker .
...
deploy_kubernetes_resources() {
    ...
    kubectl apply -f core/deployment/ai-agent-evaluator-complete.yaml -n "$NAMESPACE"
...
deploy_scheduled_jobs() {
    ...
    sed -i.bak "s|image: ai-agent-evaluator:latest|image: ${DOCKER_REGISTRY}/ai-agent-evaluator:${IMAGE_TAG}|g" core/deployment/ai-agent-evaluation-cronjob.yaml
    
    # Apply CronJob
    kubectl apply -f core/deployment/ai-agent-evaluation-cronjob.yaml -n "$NAMESPACE"
    
    # Restore original file
    mv core/deployment/ai-agent-evaluation-cronjob.yaml.bak core/deployment/ai-agent-evaluation-cronjob.yaml
...
deploy_api_server() {
    ...
    sed -i.bak "s|image: ai-agent-evaluator:latest|image: ${DOCKER_REGISTRY}/ai-agent-evaluator-api:${IMAGE_TAG}|g" core/deployment/ai-agent-evaluator-deployment.yaml
    
    # Apply deployment
    kubectl apply -f core/deployment/ai-agent-evaluator-deployment.yaml -n "$NAMESPACE"
    ...
    # Restore original file
    mv core/deployment/ai-agent-evaluator-deployment.yaml.bak core/deployment/ai-agent-evaluator-deployment.yaml
