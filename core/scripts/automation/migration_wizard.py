...
def main() -> None:
    ...
    parser.add_argument(
        "--overlay-order-file",
        default="core/operators/flux/overlay-order.txt",
    ...
    parser.add_argument(
        ...
        choices=["true", "false"],
        default="true",
        help="Run core/core/automation/ci-cd/scripts/apply-overlay-order.sh before manual ordering.",
    )
    parser.add_argument(
        "--helper-script",
        nargs="*",
        default=["./core/core/automation/ci-cd/scripts/enable-cloud.sh"],
        help="Helper scripts to invoke after ordering.",
    )
    parser.add_argument(
        "--ci-gate",
        nargs="*",
        default=["./core/core/automation/ci-cd/scripts/prerequisites.sh"],
    ...
    apply_order_script = Path("core/core/automation/ci-cd/scripts/apply-overlay-order.sh")
    temp_order = None
    ...
    if args.emulator:
        run_helper(Path(repo_path / "core/core/automation/ci-cd/scripts/enable-cloud.sh"), [args.provider, f"--emulator={args.emulator}"])
