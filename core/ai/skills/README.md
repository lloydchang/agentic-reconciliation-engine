...
python main.py provision \
  ...
  --config ../../overlay/examples/config.json
...
See `overlay/examples/config.json` for a complete configuration example:
...
COPY core/ai/skills/infrastructure-provisioning /app
WORKDIR /app
RUN pip install uv
...
1. Create skill directory: `core/ai/skills/new-skill/`
...
Use [core/ai/skills/SKILL_TEMPLATE.md](core/ai/skills/SKILL_TEMPLATE.md) as a starting point for new skills.
