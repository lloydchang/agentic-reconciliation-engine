...
2. Run `core/core/automation/ci-cd/scripts/prerequisites.sh` in your shell to validate the skill set, tooling versions, and environment variables.
3. Execute any relevant automation (e.g., `core/core/automation/ci-cd/scripts/run-local-automation.sh`, `core/core/automation/ci-cd/scripts/run-emulator-then-cloud.sh`, or `core/core/automation/ci-cd/scripts/migration_wizard.py`) from the same shell to confirm the new behavior works end to end.
...
- Whether you're running the automation from Codex, Claude Code/Windsurf, Cursor, the VS Code terminal, or another tool, the same shell requirements apply: open a POSIX shell, run `core/core/automation/ci-cd/scripts/prerequisites.sh`, and let the detection helper report `INFO [WSL check]` when you're in WSL or warn if you're still on an unsupported Windows prompt.
...
- If you add new Windows entry points or onboarding steps, make sure they source `core/core/automation/ci-cd/scripts/helpers/wsl-detect.sh` and report their environment so we keep the experience consistent for contributors on all platforms.
