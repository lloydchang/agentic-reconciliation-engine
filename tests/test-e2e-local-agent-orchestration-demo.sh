...
demo_infrastructure() {
    ...
    kubectl apply -k infrastructure/monitoring/ || print_warning "Monitoring deployment failed - continuing without monitoring"
    kubectl apply -k infrastructure/tenants/1-network/ || print_warning "Network deployment failed - continuing without network components"
    kubectl apply -k infrastructure/tenants/2-clusters/ || print_warning "Cluster deployment failed - continuing without cluster components"
...
demo_consensus_agents() {
    ...
    if kubectl get agentconsensus infrastructure-consensus -n control-plane &>/dev/null; then
        ...
        kubectl apply -k examples/complete-hub-spoke/agent-workflows/
    fi
...
demo_ai_integration() {
    ...
    if [[ "$USE_LOCAL_LLM" == "true" ]]; then
        ...
        kubectl apply -k examples/complete-hub-spoke/ai-cronjobs/
        kubectl apply -k examples/complete-hub-spoke/ai-validation/
        kubectl apply -k examples/complete-hub-spoke/agent-workflows/
    else
        ...
        kubectl apply -k examples/complete-hub-spoke/ai-cronjobs/
        kubectl apply -k examples/complete-hub-spoke/ai-validation/
        kubectl apply -k examples/complete-hub-spoke/
    fi
...
generate_demo_report() {
    ...
    echo "Demo Documentation: examples/complete-hub-spoke/agent-orchestration-demo.md"
    ...
    if [[ $passed_phases -eq $total_phases ]]; then
        ...
        echo "   • examples/complete-hub-spoke/README.md"
        echo ""
        return 0
    else
        ...
        echo "   4. Review agent configuration in examples/complete-hub-spoke/"
        echo ""
        return 1
    fi
}
...
main() {
    ...
    echo "Demo: examples/complete-hub-spoke/agent-orchestration-demo.md"
    if [[ "$USE_LOCAL_LLM" == "true" ]]; then
