...
test_validation() {
    ...
    if [[ -f "tests/test-local-suite.sh" ]]; then
        if ! ./tests/test-local-suite.sh; then
...
test_drift() {
    ...
    # Run drift test
    if [[ -f "tests/drift-test.sh" ]]; then
        if ! ./tests/drift-test.sh; then
...
generate_test_report() {
    ...
    if [[ $passed_tests -eq $total_tests ]]; then
        ...
        echo "   4. Run individual tests: ./tests/test-components.sh <component>"
        echo ""
        return 1
    fi
}
