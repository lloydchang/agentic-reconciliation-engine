...
kubectl apply -f examples/complete-hub-spoke-kagent/kagent-core/
...
kubectl apply -f examples/complete-hub-spoke-kagent/monitoring/

# Deploy MCP registries
kubectl apply -f examples/complete-hub-spoke-kagent/mcp-registry/
```

### 2.2 Create Parallel TaskSpawners
```bash
# Deploy kagent TaskSpawners alongside existing CronJobs
kubectl apply -f examples/complete-hub-spoke-kagent/taskspawners/
...
kubectl apply -f examples/complete-hub-spoke-kagent/monitoring/kagent-monitoring.yaml
...
kubectl apply -f examples/complete-hub-spoke-kagent/agent-workflows/gitops-pipelines.yaml
...
kubectl patch kustomization complete-hub-spoke-with-ai -n flux-system --type='merge' -p='{"resources":["../../examples/complete-hub-spoke-kagent/"]}'
```
...
kubectl delete -f examples/complete-hub-spoke-kagent/
...
```bash
# Add custom metrics
kubectl apply -f examples/complete-hub-spoke-kagent/monitoring/custom-metrics.yaml

# Configure alerting
kubectl apply -f examples/complete-hub-spoke-kagent/monitoring/alert-rules.yaml
```
