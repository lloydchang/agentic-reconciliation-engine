...
# Infrastructure Discovery — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/infrastructure-discovery.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud infrastructure-discovery - Agent-Executable Implementation
...
def main():
    ...
    operator = InfrastructureDiscovery()
...
```python
# MCP server handler for multi-cloud infrastructure-discovery integration
from mcp.server import Server
from mcp.types import Tool

app = Server("infrastructure-discovery")

@app.list_tools()
async def list_tools():
    return [
        Tool(
            name="infrastructure_discovery_execute",
            description="Execute multi-cloud infrastructure-discovery operation",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> str:
    if name == "infrastructure_discovery_execute":
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for infrastructure-discovery
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing infrastructure-discovery operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python infrastructure-discovery
func (a *SkillExecutionActivities) ExecuteInfrastructureDiscovery(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/infrastructure-discovery.py", string(paramsJSON))
