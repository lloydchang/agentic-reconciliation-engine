...
# Incident Triage Runbook — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/incident-triage-runbook.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud incident-triage-runbook - Agent-Executable Implementation
...
def main():
    ...
    operator = IncidentTriageRunbook()
...
```python
# MCP server handler for multi-cloud incident-triage-runbook integration
from mcp.server import Server
from mcp.types import Tool

app = Server("incident-triage-runbook")

@app.list_tools()
async def list_tools():
    return [
        Tool(
            name="incident_triage_runbook_execute",
            description="Execute multi-cloud incident-triage-runbook operation",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> str:
    if name == "incident_triage_runbook_execute":
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for incident-triage-runbook
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing incident-triage-runbook operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python incident-triage-runbook
func (a *SkillExecutionActivities) ExecuteIncidentTriageRunbook(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/incident-triage-runbook.py", string(paramsJSON))
