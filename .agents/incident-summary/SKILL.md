...
# Incident Summary — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/incident-summary.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud incident-summary - Agent-Executable Implementation
...
```python
# MCP server handler for multi-cloud incident-summary integration
from mcp.server import Server
from mcp.types import Tool

app = Server("incident-summary")

@app.list_tools()
async def list_tools():
    return [
        Tool(
            name="incident_summary_execute",
            description="Execute multi-cloud incident-summary operation",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> str:
    if name == "incident_summary_execute":
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for incident-summary
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing incident-summary operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python incident-summary
func (a *SkillExecutionActivities) ExecuteIncidentSummary(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/incident-summary.py", string(paramsJSON))
