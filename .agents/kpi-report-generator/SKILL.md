...
# Kpi Report Generator — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/kpi-report-generator.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud kpi-report-generator - Agent-Executable Implementation
...
```python
# MCP server handler for multi-cloud kpi-report-generator integration
from mcp.server import Server
from mcp.types import Tool

app = Server("kpi-report-generator")

@app.list_tools()
async def list_tools():
    return [
        Tool(
            name="kpi_report_generator_execute",
            description="Execute multi-cloud kpi-report-generator operation",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> str:
    if name == "kpi_report_generator_execute":
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for kpi-report-generator
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing kpi-report-generator operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python kpi-report-generator
func (a *SkillExecutionActivities) ExecuteKpiReportGenerator(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/kpi-report-generator.py", string(paramsJSON))
