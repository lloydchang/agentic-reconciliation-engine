...
# Network Diagnostics — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/network-diagnostics.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud network-diagnostics - Agent-Executable Implementation
...
```python
# MCP server handler for multi-cloud network-diagnostics integration
from mcp.server import Server
from mcp.types import Tool

app = Server("network-diagnostics")

@app.list_tools()
async def list_tools():
    return [
        Tool(
            name="network_diagnostics_execute",
            description="Execute multi-cloud network-diagnostics operation",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> str:
    if name == "network_diagnostics_execute":
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for network-diagnostics
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing network-diagnostics operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python network-diagnostics
func (a *SkillExecutionActivities) ExecuteNetworkDiagnostics(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/network-diagnostics.py", string(paramsJSON))
