...
# Cost Optimizer — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/cost-optimizer.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud cost-optimizer - Agent-Executable Implementation
...
```python
# MCP server handler for multi-cloud cost-optimizer integration
from mcp.server import Server
from mcp.types import Tool

app = Server("cost-optimizer")

@app.list_tools()
async def list_tools():
    return [
        Tool(
            name="cost_optimizer_execute",
            description="Execute multi-cloud cost-optimizer operation",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> str:
    if name == "cost_optimizer_execute":
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for cost-optimizer
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing cost-optimizer operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python cost-optimizer
func (a *SkillExecutionActivities) ExecuteCostOptimizer(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/cost-optimizer.py", string(paramsJSON))
