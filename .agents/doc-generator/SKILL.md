...
# Doc Generator — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/doc-generator.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud doc-generator - Agent-Executable Implementation
...
```python
# MCP server handler for multi-cloud doc-generator integration
from mcp.server import Server
from mcp.types import Tool

app = Server("doc-generator")

@app.list_tools()
async def list_tools():
    return [
        Tool(
            name="doc_generator_execute",
            description="Execute multi-cloud doc-generator operation",
            inputSchema={
                "type": "object",
                "properties": {
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for doc-generator
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing doc-generator operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python doc-generator
func (a *SkillExecutionActivities) ExecuteDocGenerator(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/doc-generator.py", string(paramsJSON))
