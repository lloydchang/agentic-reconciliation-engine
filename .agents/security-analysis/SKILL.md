...
# Security Analysis — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/security-analysis.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud security-analysis - Agent-Executable Implementation
...
```python
# MCP server handler for multi-cloud security-analysis integration
...
async def list_tools():
    return [
        Tool(
            name="security_analysis_execute",
            description="Execute multi-cloud security-analysis operation",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> str:
    if name == "security_analysis_execute":
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for security-analysis
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing security-analysis operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python security-analysis
func (a *SkillExecutionActivities) ExecuteSecurityAnalysis(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/security-analysis.py", string(paramsJSON))
