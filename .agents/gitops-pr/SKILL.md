...
# Gitops Pr — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/gitops-pr.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud gitops-pr - Agent-Executable Implementation
...
```python
# MCP server handler for multi-cloud gitops-pr integration
from mcp.server import Server
from mcp.types import Tool

app = Server("gitops-pr")

@app.list_tools()
async def list_tools():
    return [
        Tool(
            name="gitops_pr_execute",
            description="Execute multi-cloud gitops-pr operation",
            inputSchema={
                "type": "object",
                "properties": {
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for gitops-pr
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing gitops-pr operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python gitops-pr
func (a *SkillExecutionActivities) ExecuteGitopsPr(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/gitops-pr.py", string(paramsJSON))
