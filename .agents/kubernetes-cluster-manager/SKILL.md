...
# Kubernetes Cluster Manager — World-class Multi-Cloud Enterprise Automation Platform
...
## Scripts
- `scripts/kubernetes-cluster-manager.py`: Main multi-cloud automation with enterprise integration
...
#!/usr/bin/env python3
"""
World-class Multi-Cloud kubernetes-cluster-manager - Agent-Executable Implementation
...
def main():
    ...
    operator = KubernetesClusterManager()
...
```python
# MCP server handler for multi-cloud kubernetes-cluster-manager integration
from mcp.server import Server
from mcp.types import Tool

app = Server("kubernetes-cluster-manager")

@app.list_tools()
async def list_tools():
    return [
        Tool(
            name="kubernetes_cluster_manager_execute",
            description="Execute multi-cloud kubernetes-cluster-manager operation",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> str:
    if name == "kubernetes_cluster_manager_execute":
...
```bash
#!/bin/bash
# Multi-cloud CLI execution for kubernetes-cluster-manager
CLOUD_PROVIDER=${1:-all}
OPERATION=${2:-analyze}
TARGET=${3:-example}

echo "Executing kubernetes-cluster-manager operation on ${CLOUD_PROVIDER} cloud provider(s)"
...
```go
// Go activity that executes multi-cloud Python kubernetes-cluster-manager
func (a *SkillExecutionActivities) ExecuteKubernetesClusterManager(ctx context.Context, params map[string]interface{}) (interface{}, error) {
    paramsJSON, _ := json.Marshal(params)
    cmd := exec.CommandContext(ctx, "python3", "scripts/kubernetes-cluster-manager.py", string(paramsJSON))
