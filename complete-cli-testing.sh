...
run_test "Kubeconfig Access" "export KUBECONFIG=/Users/lloyd/github/antigravity/gitops-infra-control-plane/hub-kubeconfig && kubectl config use-context hub" "true"
...
# Kubernetes tests
run_test "Metrics Pod Running" "export KUBECONFIG=/Users/lloyd/github/antigravity/gitops-infra-control-plane/hub-kubeconfig && kubectl get pods -n ai-infrastructure | grep ai-metrics-server | grep Running" "true"
run_test "Metrics Service Exists" "export KUBECONFIG=/Users/lloyd/github/antigravity/gitops-infra-control-plane/hub-kubeconfig && kubectl get svc -n ai-infrastructure | grep ai-metrics-service" "true"
...
if [ $FAILED_TESTS -gt 0 ]; then
    ...
    if ! ps aux | grep -v grep | grep 'port-forward.*ai-metrics' >/dev/null; then
        apply_fix "Establish Port-Forward" "pkill -f 'port-forward.*ai-metrics' 2>/dev/null; sleep 1; export KUBECONFIG=/Users/lloyd/github/antigravity/gitops-infra-control-plane/hub-kubeconfig && kubectl config use-context hub >/dev/null 2>&1 && kubectl port-forward svc/ai-metrics-service 8080:8080 -n ai-infrastructure &"
    fi
    ...
    run_test "Post-Fix Data Flow" "curl -s --connect-timeout 5 http://localhost:5002/api/agents/detailed" "false"
fi
