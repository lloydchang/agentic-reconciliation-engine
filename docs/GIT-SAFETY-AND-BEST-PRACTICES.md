...
sed -i ''s|/tmp/gitops-bootstrap-kind-config.yaml|/tmp/${BOOTSTRAP_CLUSTER_NAME}-kind-config.yaml|g' $TOPDIR/scripts/create-bootstrap-cluster.sh
```

#### Files Affected
- `$TOPDIR/scripts/create-bootstrap-cluster.sh`
