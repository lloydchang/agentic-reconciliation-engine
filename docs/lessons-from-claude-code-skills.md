...
Internal libs, CLis, SDKs, gotchas billing-lib•platforn-cli•events

Product Verification  
Drive the running product to verify  
549p-oravercaeco волал

Data & Analysis  
IDs, field names, query patterns funnel-query • grafana • datadag

Business Automation  
Multi-tool workflows → one command  
standup: tickets • weekly-recap

Scaffolding & Templates  
Framework-correct boilerplate  
new-app nigration workflow

Code Quality & Review  
Methodology that ships better code adversarial • hypothesis • bughunt
...
Incident Runbooks  
Symptom → investigation → report oncall correlator • queue-debug

Infrastructure Ops  
Safety-gated cleanup & maintenance orphans • deps • cost-investigation
