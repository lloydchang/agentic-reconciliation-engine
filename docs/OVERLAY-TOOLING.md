...
```bash
# Make executable
chmod +x core/scripts/automation/overlay-cli.py
...
python core/scripts/automation/overlay-cli.py.py create dark-theme dashboard themes --template dashboard-overlay
...
python core/scripts/automation/overlay-cli.py create enterprise-bundle composed ""
```
...
python core/scripts/automation/overlay-cli.py test core/deployment/overlays/core/ai/skills/debug/enhanced

# Test composition
python core/scripts/automation/overlay-cli.py test core/deployment/overlays/composed/community-bundle
```
...
python core/scripts/automation/overlay-cli.py build core/deployment/overlays/core/ai/skills/debug/enhanced
...
# Search by tags
python core/scripts/automation/overlay-cli.py search "ml" --tags machine-learning debugging

# Search with filters
python core/scripts/automation/overlay-cli.py search "enhanced" --category skills
```
...
python core/scripts/automation/overlay-registry.py register core/deployment/overlays/core/ai/skills/debug/enhanced

# Register with custom metadata
python core/scripts/automation/overlay-registry.py register core/deployment/overlays/my-overlay --metadata custom-metadata.yaml
```
...
python core/scripts/automation/registry.py validate
```
...
python core/scripts/automation/validate-overlays.py core/deployment/overlays/core/ai/skills/debug/enhanced
...
```bash
# Test single overlay
python core/scripts/automation/test-overlays.py core/deployment/overlays/core/ai/skills/debug/enhanced
...
on:
  push:
    paths:
      ...
      - 'core/scripts/automation/validate-overlays.py'
  pull_request:
    paths:
      - 'core/deployment/overlays/**'
...
```python
# Start MCP server
python core/scripts/automation/overlay-webmcp.py --port 8080
...
ci:
  - python core/scripts/automation/test-overlays.py core/deployment/overlays/
  - python core/scripts/automation/overlay-cli.py build core/deployment/overlays/community-bundle
```
...
# VS Code task
{
  ...
  "type": "shell",
  "command": "python core/scripts/automation/validate-overlays.py ${workspaceFolder}/overlays"
}
```
...
def validate_and_build_overlay(overlay_path):
    ...
    result = subprocess.run([
        'python', 'core/scripts/automation/validate-overlays.py', overlay_path
    ])
    ...
    # Build
    result = subprocess.run([
        'python', 'core/scripts/automation/overlay-cli.py', 'build', overlay_path
    ])
    if result.returncode != 0:
        print("Build failed")
        return False
...
# Check dependencies
python core/scripts/automation/test-overlays.py core/deployment/overlays/ --test-dependencies
```
