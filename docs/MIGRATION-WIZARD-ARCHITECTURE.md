...
- **Orchestrator (`core/scripts/automation/migration_wizard.py`)**: Drives the ordered overlay list, invokes helper scripts (`core/scripts/automation/enable-cloud.sh`, export/migrate helpers), runs the CI policy gate (Conftest + kubeconform), and communicates status to the user.
...
- **Emulator toggles**: The wizard can enable/disable the Azure/GCP/AWS emulator entries automatically by calling `core/scripts/automation/enable-cloud.sh <provider> --emulator=enable|disable` or the generic `--toggle` option for other lines.
- **Automation hooks**: The wizard reuses existing helper scripts and will call future ones (`core/scripts/automation/apply-overlay-order.sh`, `core/scripts/automation/overlay-logician.py`) to fulfill tasks like ordering, validation, and state export.
...
- **Git host connectors**: Pluggable adapters handle cloning, branching, pushing, and opening merge requests against GitHub Enterprise, GitLab, Azure DevOps, Bitbucket, Gitea, CodeCommit, or any other Git service. The wizard delegates all Git operations to the connector via a stable interface (`clone()`, `push()`, `create_pr()`), so adding a new platform is a matter of implementing that interface.
  ...
  - For hosts that can’t open PRs in-platform (CodeCommit, Secure Source Manager), use `core/scripts/automation/open-gh-console-pr.sh` after the wizard finishes to get the console URL where you can create the pull request manually.
...
4. Run helper scripts (`core/scripts/automation/enable-cloud.sh`, `core/scripts/automation/export-argocd-state.sh`, `core/scripts/automation/migrate-app.sh`) per the migration playbooks.
...
- Use `core/scripts/automation/migration_wizard.py` directly from the EKS/AKS/GKE playbooks (or other migration runbooks) to keep overlay ordering, emulator toggles, and CI validation automated. Each runbook can call the wizard with the desired overlay order, helper scripts, CI gate command, and connector flag (e.g., `--connector=gitlab --overlay-order ./bootstrap ./hub ./cloud-azure ...`). Because the wizard reorders overlays itself and exposes the connector interface, it becomes the automation helper for every migration step, ensuring the documented flow executes in one guided session.
...
- Additional automation (e.g., the `core/scripts/automation/overlay-logician.py` verification tool) can be wired into the wizard's flow without changing the core logic, keeping the architecture modular and maintainable.
