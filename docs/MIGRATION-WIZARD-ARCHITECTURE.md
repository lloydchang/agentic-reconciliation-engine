...
- **CI gate executor**: Wraps the same policy checks referenced in `docs/CI-POLICY-GATE.md` so every migration path runs schema/annotation/policy validation before changes are pushed.
