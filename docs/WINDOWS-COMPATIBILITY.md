...
bash-aware shell. The underlying scripts (`core/scripts/automation/prerequisites.sh`, `core/scripts/automation/migration_wizard.py`,
`core/scripts/automation/run-local-automation.sh`, etc.) rely on familiar UNIX utilities (`bash`, `mkdir -p`, `tee`,
...
- Export the necessary environment variables inside WSL or Git Bash the same way as on macOS/Linux.
  You can source `core/scripts/automation/prerequisites.sh` while remaining in the same shell, ensuring the script can see the values.
...
The helper script runs `core/scripts/automation/prerequisites.sh`, which validates CLI tooling, and then calls
`core/scripts/automation/migration_wizard.py` with the Azure emulator + GitHub connector sequence. Because both scripts
...
1. Run `core/scripts/automation/prerequisites.sh` from your Windows shell; everything should pass or fail with clear
   diagnostics (paths, env vars, CLI tools).  
2. Run `core/scripts/automation/run-local-automation.sh`; the logs under `logs/local-core/automation/ci-cd/` provide the same
   output as on macOS/Linux.  
3. Because Windows uses the same bash environment, this also validates the `core/scripts/automation/migration_wizard.py`
...
The main automation entry points (`core/scripts/automation/prerequisites.sh`, `core/scripts/automation/run-local-automation.sh`, and
`core/scripts/automation/run-emulator-then-cloud.sh`) now source `core/scripts/automation/helpers/wsl-detect.sh`. That helper logs a warning
