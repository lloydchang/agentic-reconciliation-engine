...
Because macOS shells understand `bash`, `mkdir`, `tee`, `python`, etc., the same zero-touch script described in the README works out of the box. The script logs to `logs/local-automation/` so you can validate each phase (bootstrap check, migration wizard run, CI gate).
