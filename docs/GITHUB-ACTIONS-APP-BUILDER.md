...
1. **Immutable Tags**: The workflow generates sortable, timestamp-based tags that work with Flux's ImagePolicy resources
2. **Mutable Tags**: Optional `canary` and `latest` tags for environments that require mutable references
3. **Automation Ready**: Tags are structured for Flux's image update automation
...
Last modified 2024-07-10: Update GitHub Actions versions (c148323)
