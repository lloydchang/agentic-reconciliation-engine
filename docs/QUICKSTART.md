...
6. **Review logs.** Check `logs/local-automation/latest-summary.json` and the `.log` files in `logs/local-automation/` to confirm the configuration setup completed and to troubleshoot failures.
