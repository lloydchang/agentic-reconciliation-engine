...
The automation in this repository requires a POSIX-style shell that understands `bash` syntax. Scripts such as `core/scripts/automation/prerequisites.sh`, `core/scripts/automation/migration_wizard.py`, `core/scripts/automation/run-local-automation.sh`, and the helper utilities depend on:
...
- **macOS**: The default zsh shell understands these scripts, but you can install GNU bash via Homebrew (`brew install bash`) and run the scripts explicitly with `bash core/scripts/automation/...`. Set `SHELL=/bin/bash` if needed.  
...
- **Python**: `python3` must be installed; `core/scripts/automation/migration_wizard.py` invokes the system Python interpreter. Ensure the shell’s PATH resolves `python` to the Python 3 binary.
