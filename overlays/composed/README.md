...
kustomize build overlays/composed/enterprise-variant | kubectl apply -f -

# Deploy open source variant
kustomize build overlays/composed/opensource-variant | kubectl apply -f -
...
kustomize build overlays/composed/enterprise-suite | kubectl apply --dry-run=client -f -

# Validate composition
python scripts/validate-overlays.py overlays/composed/enterprise-suite

# Test composition
python scripts/test-overlays.py overlays/composed/enterprise-suite
```
...
### From `variants/enterprise/` → `overlays/composed/enterprise-variant/`
- All enterprise features preserved
- Enhanced with overlay system capabilities
- Better dependency management
- Improved testing and validation

### From `variants/opensource/` → `overlays/composed/opensource-variant/`
...
1. **Create Directory Structure**:
   ```bash
   mkdir -p overlays/composed/my-composed-overlay
   ```
...
4. **Test and Validate**:
   ```bash
   python scripts/validate-overlays.py overlays/composed/my-composed-overlay
   python scripts/test-overlays.py overlays/composed/my-composed-overlay
   ```
...
python scripts/validate-overlays.py overlays/composed/my-overlay

# Test composition
python scripts/test-overlays.py overlays/composed/my-overlay

# Check build output
kustomize build overlays/composed/my-overlay

# Dry run deployment
kustomize build overlays/composed/my-overlay | kubectl apply --dry-run=client -f -
```
