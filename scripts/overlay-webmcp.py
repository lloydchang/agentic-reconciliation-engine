    ...
    async def _create_overlay(self, params: Dict[str, Any]) -> OverlayResponse:
        ...
        try:
            ...
            if result.returncode == 0:
                return OverlayResponse(
                    success=True,
                    data={"message": f"Overlay {name} created successfully", "path": f"overlays/{category}/{name}"}
                )
            else:
                return OverlayResponse(
                    success=False,
