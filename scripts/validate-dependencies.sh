...
test_automated_debugging_scripts() {
    ...
    if [[ -f "./scripts/debug-dependency-chain.sh" && -x "./scripts/debug-dependency-chain.sh" ]]; then
    ...
    if [[ -f "./scripts/validate-dependencies.sh" && -x "./scripts/validate-dependencies.sh" ]]; then
    ...
    if [[ -f "./scripts/generate-controller-config.sh" && -x "./scripts/generate-controller-config.sh" ]]; then
    ...
    if [[ -f "./scripts/setup-multi-hub.sh" && -x "./scripts/setup-multi-hub.sh" ]]; then
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        if ./scripts/debug-dependency-chain.sh --help > /dev/null 2>&1; then
