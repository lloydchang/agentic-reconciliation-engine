...
if kubectl exec -n ai-infrastructure deployment/ai-metrics-server -- curl -s http://localhost:8080/api/agents/status > /dev/null; then
...
echo ""
echo "📊 Access URLs:"
echo "   Metrics API: http://localhost:30080/api/agents/status"
...
echo "   Access the dashboard: ./scripts/smart-dashboard.sh"
