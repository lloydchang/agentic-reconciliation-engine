...
validate_staged_agents() {
    ...
    local changed_dirs=$(git diff --cached --name-only --diff-filter=d | grep -E "^[^/]+\.agents/[^/]+/$" | sed 's|/||' | sed 's|/$||')
    ...
    if [ $violations -gt 0 ]; then
        ...
        print_status "INFO" "Run './scripts/ensure-agent-naming-standards.sh fix' to correct issues."
        exit 1
    else
