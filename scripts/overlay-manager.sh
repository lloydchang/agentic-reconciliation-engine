...
list_overlays() {
    ...
    while IFS= read -r -d '' overlay; do
        if [[ -f "$overlay/kustomization.yaml" ]]; then
            ...
            if [[ "$overlay_name" == *".agents/"* ]]; then
                overlay_type="skill"
            elif [[ "$overlay_name" == *".agents/dashboard"* ]]; then
                overlay_type="dashboard"
            elif [[ "$overlay_name" == *"control-plane/"* ]]; then
...
create_overlay() {
    ...
    case "$overlay_type" in
        "skill")
            target_dir="$OVERLAY_DIR/.agents/$base_path"
            ;;
        "dashboard")
            target_dir="$OVERLAY_DIR/agents/dashboard/$base_path"
            ;;
        "infrastructure")
            target_dir="$OVERLAY_DIR/control-plane/$base_path"
            ;;
        "composed")
...
get_base_resources() {
    ...
    case "$overlay_type" in
        "skill")
            echo "../../.agents/$base_path"
            ;;
        "dashboard")
            echo "../../.agents/dashboard/$base_path"
            ;;
        "infrastructure")
            echo "../../control-plane/$base_path"
            ;;
        "composed")
            echo ""
            ;;
