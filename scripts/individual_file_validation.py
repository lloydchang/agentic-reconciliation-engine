#!/usr/bin/env python3
"""
Individual File Validation - Check each SKILL.md file one by one
"""

import yaml
from pathlib import Path

def validate_single_file(file_path: Path, file_name: str) -> dict:
    """Validate a single file with detailed checking"""
    try:
    ...
    # Check API Patterns section quality
...
def main():
    """Check each and every file individually"""
    agents_dir = Path(__file__).parent.parent / ".agents"
    
    print(f"🔍 INDIVIDUAL FILE VALIDATION")
    print(f"=" * 50)
    print(f"Checking each SKILL.md file individually...")
    ...
    for i, file_path in enumerate(skill_files, 1):
        ...
        result = validate_single_file(file_path, file_name)
    ...
    if agentskills_compliant == total_count and agents_md_compliant == total_count:
        ...
        print(f"❌ Q1: {total_count - agentskills_compliant} files don't follow agentskills.io")
    ...
    if non_compliant:
        ...
        for result in non_compliant:
            print(f"   • {result['file_name']}: {result['issues'][0]}")
    else:
