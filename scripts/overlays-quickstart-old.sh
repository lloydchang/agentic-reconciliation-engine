...
create_examples() {
    ...
    cat > "overlays/.agents/debug/hello-world/patches/hello-world.yaml" << 'EOF'
---
apiVersion: v1
kind: ConfigMap
metadata:
...
EOF
    ...
    # Add dark theme CSS
    mkdir -p "overlays/agents/dashboard/themes/dark-theme/theme"
    cat > "overlays/agents/dashboard/themes/dark-theme/theme/dark.css" << 'EOF'
:root {
...
EOF
    ...
    cat > "overlays/composed/production-env/kustomization.yaml" << 'EOF'
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization
metadata:
...
EOF
    ...
    mkdir -p "overlays/control-plane/multi-cloud/configs"
    
    # AWS configuration
    cat > "overlays/control-plane/multi-cloud/configs/aws.yaml" << 'EOF'
apiVersion: v1
kind: ConfigMap
metadata:
  name: aws-config
...
EOF
    ...
    # Azure configuration
    cat > "overlays/control-plane/multi-cloud/configs/azure.yaml" << 'EOF'
apiVersion: v1
kind: ConfigMap
metadata:
  name: azure-config
...
EOF
    ...
    # GCP configuration
    cat > "overlays/control-plane/multi-cloud/configs/gcp.yaml" << 'EOF'
apiVersion: v1
kind: ConfigMap
metadata:
  name: gcp-config
