...
# Ensure remove_age_key_and_reencrypt.sh exists in scripts/
if [ ! -f "$SCRIPT_DEST_DIR/$SCRIPT_NAME" ]; then
    echo "$SCRIPT_NAME not found in $SCRIPT_DEST_DIR, downloading from GitHub..."
    curl -sL "https://raw.githubusercontent.com/lloydchang/gitops-infra-control-plane/main/scripts/$SCRIPT_NAME" -o "$SCRIPT_DEST_DIR/$SCRIPT_NAME"
